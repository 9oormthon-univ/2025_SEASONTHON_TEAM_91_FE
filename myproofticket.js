...
import { listMyTickets as getMyTickets, getTicketDetail as getTicketById } from './js/apiClient.js';
  ...
  try {
    ...
    RAW = items.map((item, i) => ({
      ...
      contest: {
        ...
        imageUrl: item.contest_image_url || postersFallback[i % postersFallback.length],
      },
      ticketDetail: {
        humanCode: item.human_code,
        issuedAt: item.issued_at,
        fileNameSubmitted: item.file_name_submitted,
        owner: {
          name: JSON.parse(localStorage.getItem('me') || '{}').name || '익명 사용자',
        },
      },
    }));
...
function initWheel(tickets) {
  ...
  const layout = () => {
    if (!tickets.length) return console.warn('[layout] 티켓 없음');
  ...
  const snapToNearest = (duration = 260) => {
    let best = 0, bestAbs = 1e9;
    SLOT_ANGLES.forEach((a, i) => {
      const curr = a + residual;
      const d = Math.abs(shortestDelta(TARGET, norm360(curr)));
    ...
    const start = residual, end = residual + delta, t0 = performance.now();
    const ease = (t) => 1 - Math.pow(1 - t, 3);
